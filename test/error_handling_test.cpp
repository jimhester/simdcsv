...
TEST(ParseErrorTest, ToString) {
    ParseError error(ErrorCode::INCONSISTENT_FIELD_COUNT, ErrorSeverity::ERROR,
                     3, 1, 50, "Expected 3 fields but found 2", "1,2");
...
TEST(ErrorCollectorTest, AddErrorConvenience) {
    ...
    collector.add_error(ErrorCode::QUOTE_IN_UNQUOTED_FIELD, ErrorSeverity::ERROR,
                        3, 5, 45, "Invalid quote", "bad\"quote");
...
TEST(ErrorCollectorTest, WarningsDontStopParsing) {
    ...
    collector.add_error(ErrorCode::MIXED_LINE_ENDINGS, ErrorSeverity::WARNING,
                        1, 1, 10, "Mixed line endings detected");
    ...
    collector2.add_error(ErrorCode::MIXED_LINE_ENDINGS, ErrorSeverity::WARNING,
                         1, 1, 10, "Mixed line endings detected");
...
TEST(ParseExceptionTest, MultipleErrors) {
    ...
    errors.emplace_back(ErrorCode::INCONSISTENT_FIELD_COUNT, ErrorSeverity::ERROR,
                        1, 1, 10, "Error 1");
    errors.emplace_back(ErrorCode::QUOTE_IN_UNQUOTED_FIELD, ErrorSeverity::ERROR,
                        2, 1, 20, "Error 2");
...
TEST_F(MalformedCSVTest, TrailingQuoteExists) {
    ...
    EXPECT_NE(content.find("6\""), std::string::npos)
        << "Should have quote after unquoted field";
}
...
TEST_F(MalformedCSVTest, TripleQuoteExists) {
    ...
    EXPECT_NE(content.find("\"\"\""), std::string::npos)
        << "Should contain triple quote sequence";
}
