 ...
 * mechanisms used throughout the libvroom library. The framework supports four
 * error handling modes (DISABLED, FAIL_FAST, PERMISSIVE, BEST_EFFORT) to
 * accommodate different use cases from maximum performance to strict validation.
 *
 ...
 * - DISABLED: Maximum performance, no error collection (default)
 * - FAIL_FAST: Best for data validation, stops immediately on any error
...
public:
   ...
   *
   * @param mode Error handling mode (FAIL_FAST, PERMISSIVE, BEST_EFFORT, or DISABLED)
   * @param max_errors Maximum errors to collect (default: 10000)
   *
   * @note The default is FAIL_FAST for ErrorCollector when explicitly created.
   *       For CsvOptions, the default is DISABLED for zero-overhead parsing.
   */
   ...
   */
  bool is_enabled() const { return mode_ != ErrorMode::DISABLED; }
 ...
 * @code
 * try {
 *     auto result = convert_csv_to_parquet(input, output, opts);
...
/**
 * @brief Convert an ErrorMode to its string representation.
 *
 * @param mode The ErrorMode to convert
 * @return C-string name of the mode (e.g., "DISABLED", "FAIL_FAST")
 */
