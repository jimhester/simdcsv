...
public:
  ...
  static std::string generate_nyc_taxi_data(size_t num_rows) {
    ...
    for (size_t i = 0; i < num_rows; ++i) {
      ...
      ss << vendor_dist(gen) << ",";
      ss << "2024-01-" << std::setfill('0') << std::setw(2) << ((i % 30) + 1) 
         << " " << std::setw(2) << (i % 24) << ":" << std::setw(2) << (i % 60) << ":00,";
      ss << "2024-01-" << std::setfill('0') << std::setw(2) << ((i % 30) + 1) 
         << " " << std::setw(2) << ((i + 1) % 24) << ":" << std::setw(2) << ((i + 30) % 60) << ":00,";
  ...
  static std::string generate_financial_data(size_t num_rows, size_t num_cols = 8) {
    ...
    for (size_t i = 0; i < num_rows; ++i) {
      ss << "2024-01-01T" << std::setfill('0') << std::setw(2) << (i % 24) 
         << ":" << std::setw(2) << (i % 60) << ":00,";
      ss << "STOCK" << (i % 100) << ",";
  ...
  static std::string generate_genomics_data(size_t num_rows, size_t num_cols = 10) {
    ...
    // Header
    ss << "seq_id,chromosome,position,ref_base,alt_base,quality_score,coverage,frequency,annotation,sample_id\n";
  ...
  static std::string generate_log_data(size_t num_rows, size_t num_cols = 6) {
    ...
    for (size_t i = 0; i < num_rows; ++i) {
      ss << "2024-01-01T" << std::setfill('0') << std::setw(2) << (i % 24) 
         << ":" << std::setw(2) << (i % 60) << ":" << std::setw(2) << (i % 60) << ",";
...
BENCHMARK(BM_nyc_taxi_data)
  ->RangeMultiplier(10)->Range(1000, 1000000) // 1K to 1M rows (like real taxi data)
  ->Unit(benchmark::kMillisecond);
...
BENCHMARK(BM_log_data)
  ->RangeMultiplier(10)->Range(1000, 500000)
  ->Unit(benchmark::kMillisecond);
