...
For scenarios where determining exact quote state is expensive, simdcsv supports **speculative parsing** (Chang et al., SIGMOD 2019):
...
For maximum performance, simdcsv provides a branchless state machine implementation (`branchless_state_machine.h`) that uses lookup tables instead of switch statements:
...
simdcsv uses [Google Highway](https://github.com/google/highway) (v1.3.0) for portable SIMD operations across different architectures:
...
When parsing in parallel, quoted fields may span chunk boundaries. simdcsv handles this through:
...
The simdcsv architecture draws from these foundational papers:
...
2. **Langdale, G. & Lemire, D.** (2019).
   *Parsing Gigabytes of JSON per Second*.
   ...
   Introduces the two-stage SIMD parsing architecture, carry-less multiplication for quote parity, and branchless design patterns used throughout simdcsv.
...
3. **van den Burgh, G.J.J. & Nazabal, A.** (2019).
   *Wrangling Messy CSV Files by Detecting Row and Type Patterns*.
   ...
   Provides the basis for simdcsv's dialect detection algorithm (CleverCSV approach).
