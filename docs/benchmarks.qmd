...
## Comparative Performance: simdcsv vs zsv

The following benchmarks compare simdcsv against [zsv](https://github.com/liquidaty/zsv), a high-performance CSV parser. Tests were run on Apple Silicon (M3 Max, 14 cores) using `hyperfine` for accurate measurements.

### Single-Threaded Performance

simdcsv's SIMD-optimized parsing provides significant speedups over zsv in single-threaded mode:

| File Size | simdcsv Time | simdcsv Throughput | zsv Time | zsv Throughput | Speedup |
|-----------|--------------|-------------------|----------|----------------|---------|
...
- simdcsv achieves **3-5 GB/s** single-threaded throughput on Apple Silicon
...
|--------|---------------|------|------------|
| simdcsv | 1 thread | 22.9 ms | 4.4 GB/s |
| simdcsv | 4 threads | 18.2 ms | 5.5 GB/s |
| simdcsv | 8 threads | 18.0 ms | 5.6 GB/s |
...
- simdcsv's single-threaded performance often matches or exceeds multi-threaded zsv
- zsv's `--parallel` mode with auto-detection is highly optimized for its architecture
- At 4 threads, simdcsv is **1.7x faster** than zsv (18.2 ms vs 31.3 ms)
...
| Threads | simdcsv Time | Throughput | Scaling |
|---------|--------------|------------|---------|
...
hyperfine --warmup 2 \
  "./build/scsv count -t 1 $TEMP_DIR/test_100mb.csv" \
