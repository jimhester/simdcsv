...
int main() {
    ...
    libvroom::Parser parser;
    libvroom::Dialect dialect = libvroom::Dialect::csv();  // Standard CSV
    // Or: libvroom::Dialect::tsv() for tab-separated
    // Or: libvroom::Dialect::semicolon() for semicolon-separated
    // Or: libvroom::Dialect::pipe() for pipe-separated
...
libvroom::ErrorCollector strict_errors(libvroom::ErrorMode::STRICT);

// Permissive mode - collect all errors, try to recover
libvroom::ErrorCollector permissive_errors(libvroom::ErrorMode::PERMISSIVE);

// Best-effort mode - ignore errors, parse what's possible
libvroom::ErrorCollector best_effort(libvroom::ErrorMode::BEST_EFFORT);
```
