...
class TimingAccumulator {
 public:
  ...
  std::vector<uint64_t>
      temp_result_vec;  // reused rather than allocated in the middle of timing
  int num_phases;
  int num_events;
  int fd;
  bool working;
  ...
  explicit TimingAccumulator(int num_phases_in, std::vector<int> config_vec)
    ...
    attribs.type = PERF_TYPE_SOFTWARE;  // for now
  ...
  ~TimingAccumulator() { close(fd); }

  void report_error(const std::string& context) {
    if (working) {
      std::cerr << (context + ": " + std::to_string(errno) + std::string(strerror(errno)))
                << std::endl;
    }
    working = false;
  }
...
// a that is designed to start counting on coming into scope and stop counting, putting
// its results into an Accumulator, when leaving scope. Optional - can just use explicit
// start/stop
class TimingPhase {
 public:
  ...
  ~TimingPhase() { acc.stop(phase_number); }
};
#endif  // __linux__
